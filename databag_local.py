...
def construct_cmd(op, container, bag_name, json_tmp_file_name):
  ...
  """
  cmd = "cd ~/cookbooks/chef; bundle exec knife data bag %s %s %s" % (op, container, bag_name)
  secret = "--secret-file %s" % (secret_file)
  cmd = "%s %s -F json > %s; cd -;" % (cmd, secret, json_tmp_file_name)
  return cmd
...
def search_all(container, environment="_default", search_key="db_host", search_term=""):
  ...
  for bag_name in all_clients:
    ...
    if search_key not in env_bag:
      ...
      continue
    if type(search_term) is not type(str):
      if not env_bag[search_key] == search_term:
        print "Non-regular string key not found"
        continue
    else:
    ...
    with open('matching_bags.json', "w") as fp:
...
def create_from_file(container, bag_name, json_tmp_file_name):
  ...
  """
  secret = "--secret-file %s" % secret_file
  cmd = "cd ~/cookbooks/chef; bundle exec knife data bag from file {0} '{1}' {2}; cd -;".format(container, json_tmp_file_name, secret)
...
if __name__ == '__main__':
  ...
  # key = "hosts"
  # host = ["web01.nmdev.us"]
  # new_hosts = ["web01.nmdev.us", "web02.nmdev.us"]
  # for bag in search_all(container="nmdhosting", environment=environment, search_key=key, search_term=host):
  ...
  #     key=key,
  #     term=new_hosts)
  change_value("test123456fsdlkjf", environment="staging", key="db_host", term="fake_db")
