...
PRIVATEIP=$(knife exec -E "nodes.find('chef_environment:staging AND roles:web2') { |n| puts n.name }" --config /var/jenkins_home/workspace/jenkins-chef-client/.chef/knife.rb | sed -n '1p' | awk '{print $1}')
